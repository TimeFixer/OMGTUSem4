...
def main(dataset_path, new_data_path):
    ...
    data['figure1'] = data['figure1'].map(figure_dict).fillna(0)  # Заменяем NaN на 0
    data['figure2'] = data['figure2'].map(figure_dict).fillna(0)
    ...
    for model_name, model in models.items():
        ...
        print("Значения figure1 в new_data:", new_data['figure1'].unique())
        print("Значения figure2 в new_data:", new_data['figure2'].unique())
        # Применение словаря с обработкой NaN
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: script.py <dataset_path> <new_data_path>")
        sys.exit(1)
